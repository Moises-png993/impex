  ...
  <q-layout view="lHh Lpr lFf">
    <div class="row q-mt-md">
      <div class="col-6 q-pl-md">
        ...
        <!-- Mueve la tabla aquí para que esté debajo del input y del botón -->
        <q-table
          :rows="tableData"
          :columns="columns"
          row-key="contenedor"
...
export default {
  ...
  methods: {
    buscarEstilo() {
      const url = `http://localhost:3000/api/eta-semana?estilo=${encodeURIComponent(
        this.estilo
      )}`;
      fetch(url)
    ...
    buscarContenedor() {
      const url = `http://localhost:3000/api/estilo-cantidad?contenedor=${encodeURIComponent(
        this.contenedor
      )}`;
      fetch(url)
        .then((response) => response.json())
